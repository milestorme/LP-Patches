...
void bot_minion_ai::_calculatePos(Position& pos)
{
    ...
    if (std::abs(std::abs(myangle) - std::abs(angle)) > M_PI/3.f)
        myangle = angle;
    else
        angle = myangle;
...
void bot_minion_ai::_getBotDispellableAuraList(Unit* target, Unit* caster, uint32 dispelMask, DispelChargesList& dispelList) const
{
    ...
    for (Unit::AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
    {
        ...
        if (aura->GetSpellInfo()->GetDispelMask() & dispelMask)
        {
            ...
            if (charges > 0)
            {
                DispelableAura ref(aura, 100, charges);
                dispelList.push_back(ref);
            }
        }
    }
}
...
void bot_ai::_listAuras(Player* player, Unit* unit) const
{
    ...
    ch.PSendSysMessage("crit: %.2f pct", unit->GetUnitCriticalChanceAgainst(BASE_ATTACK, me));
...
void bot_minion_ai::SetStats(bool force, bool shapeshift)
{
    ...
    //DAMAGE
    // classstats->GenerateBaseDamage(me->GetCreatureTemplate());
    // check By XEQT
    if (master->getLevel() < 58)
    {
        me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, classstats->BaseDamage[0]);
        me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, classstats->BaseDamage[0] * 1.5f);
    }
    else if (master->getLevel() > 57 && master->getLevel() < 68)
    {
        me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, classstats->BaseDamage[1]);
        me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, classstats->BaseDamage[1] * 1.5f);
    }
    else
    {
        me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, classstats->BaseDamage[2]);
        me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, classstats->BaseDamage[2] * 1.5f);
    }
    ...
    // me->UpdateLevelDependantStats(); Checking by XEQT
...
void bot_pet_ai::SetStats(bool force, bool /*unk*/)
{
    ...
    if (CanCrit())
    {
        value = m_creatureOwner->GetUnitCriticalChanceAgainst((BASE_ATTACK), me);
        crit = value * crit_mod;
    }
...
Unit* bot_ai::_getTarget(bool byspell, bool ranged, bool &reset) const
{
    ...
    for (uint8 i = 0; i != 2; ++i)
    {
        if (!t)
        {
            ...
            Cell::VisitAllObjects(me, searcher, maxdist); // me->Object VisitNearbyObject(maxdist, searcher);
        }
    }
...
void bot_ai::CalculateAttackPos(Unit* target, Position& pos) const
{
    ...
    for (uint8 i = 0; i != 5; ++i)
    {
        target->GetNearPoint(me, x, y, z, /*me->GetCombatReach()/2.f,*/ dist, angle);
...
bool bot_ai::MoveBehind(Unit &target) const
{
    ...
    {
        float x(0),y(0),z(0);
        target.GetNearPoint(me, x, y, z, /*me->GetCombatReach()/3,*/ 0.1f, me->GetAbsoluteAngle(&target));
...
void bot_minion_ai::_updateMountedState()
{
    ...
    {
        //const_cast<CreatureTemplate*>(me->GetCreatureTemplate())->GetMovementTemplate().ToString().c_str() &= ~INHABIT_AIR;
    ...
    {
        uint32 mount = 0; // master->GetMountID();
        ...
        if (!mounts.empty())
        {
            ...
            if (sGameEventMgr->IsActiveEvent(GAME_EVENT_WINTER_VEIL))
                ...
                for (Unit::AuraEffectList::const_iterator itr = mounts.begin(); itr != mounts.end(); ++itr)
                {
                    mount = (*itr)->GetId();
                }
        ...
        if (mount)
        {
            if (me->getLevel() > 19 && !GetSpell(33388))
                InitSpellMap(33389, true); //learn
            ...
            if (doCast(me, mount))
            {
                // me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_MOUNT);
                // if (master->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED))
                //    me->SetSpeedRate(MOVE_RUN, master->GetSpeedRate(MOVE_FLIGHT) * 1.37f);

                // me->RemoveAurasWithInterruptFlags(AURA_INTERRUPT_FLAG_MOUNT);
                // if (me->IsMounted())
...
bool bot_ai::InDuel(Unit const* target) const
{
    ...
    bool isbot = target->ToCreature() && target->GetTypeId() == TYPEID_UNIT && target->ToCreature()->GetBotAI();
...
void bot_minion_ai::_OnMeleeDamageUpdate(uint8 myclass) const
{
    ...
    for (uint8 i = 0; i != MAX_EQUIPMENT_ITEMS; ++i)
    {
        ...
        me->SetStatFlatModifier(UnitMods(UNIT_MOD_DAMAGE_MAINHAND + i), BASE_VALUE, weap_damage_base); // _getBotStat(i, BOT_ITEM_MOD_DAMAGE));
    }
...
void bot_minion_ai::Evade(bool force)
{
    ...
    if (master->IsInCombat() && !IAmFree())
    {
        ...
        {
            GuidSet Set;
            // std::vector<ThreatReference*> ref = master->GetThreatManager().GetModifiableThreatList();
            for (auto ref : master->GetThreatManager().GetUnsortedThreatList()) // for (std::vector<ThreatReference*>::iterator ref = master->GetThreatManager().GetModifiableThreatList().begin(); ref != master->GetThreatManager().GetModifiableThreatList().end(); ++ref)
            {
                Set.insert(ref->GetOwner()->GetGUID());
                Set.insert(ref->GetVictim()->GetGUID()); //getUnitGuid());
                // ref = ref->next();
            }
            for (GuidSet::const_iterator i = Set.begin(); i != Set.end(); ++i)
            {
                ...
                {   
                    master->GetThreatManager().ClearThreat(unit);
                    unit->GetThreatManager().ClearThreat(master);
                }
            }
        }
    ...
    if (!master->IsInCombat() || IAmFree())
    {
        if (IAmFree())
        {
            ...
            {
                GuidSet Set;
                for (auto ref : me->GetThreatManager().GetUnsortedThreatList()) // for (std::vector<ThreatReference*>::iterator ref = me->GetThreatManager().GetModifiableThreatList().begin(); ref != me->GetThreatManager().GetModifiableThreatList().end(); ++ref)
                {
                    Set.insert(ref->GetOwner()->GetGUID());
                    Set.insert(ref->GetVictim()->GetGUID()); //getUnitGuid());
                                                                // ref = ref->next();
                ...
                for (GuidSet::const_iterator i = Set.begin(); i != Set.end(); ++i)
                {
                    ...
                    if (unit && (unit->IsFriendlyTo(me) || IsInBotParty(unit) || !unit->IsInCombat()))
                    {
                        me->GetThreatManager().ClearThreat(unit);
                        // unit->GetThreatManager().ClearThreat(master);
                    }
                }
            ...
            if (Creature* m_botsPet = me->GetBotsPet())
            {
                ...
                if (!m_botsPet->GetThreatManager().IsThreatListEmpty())
                {
                    GuidSet Set;
                    for (auto ref : m_botsPet->GetThreatManager().GetUnsortedThreatList()) // for (std::vector<ThreatReference*>::iterator ref = m_botsPet->GetThreatManager().GetModifiableThreatList().begin(); ref != m_botsPet->GetThreatManager().GetModifiableThreatList().end(); ++ref)
                    {
                        Set.insert(ref->GetOwner()->GetGUID());
                        Set.insert(ref->GetVictim()->GetGUID()); //getUnitGuid());
                                                                    // ref = ref->next();
                    ...
                    for (GuidSet::const_iterator i = Set.begin(); i != Set.end(); ++i)
                    {
                        ...
                        if (unit && (unit->IsFriendlyTo(me) || IsInBotParty(unit) || !unit->IsInCombat()))
                        {
                            m_botsPet->GetThreatManager().ClearThreat(unit);
                            // unit->GetThreatManager().ClearThreat(master);
                        }
                    }
                    /*
        ...
        for (BotMap::const_iterator it = map->begin(); it != map->end(); ++it)
        {
            Creature* cre = it->second;
            if (!cre) continue;
            if (!cre->GetIAmABot()) continue;
            if (cre->IsInCombat())
            {
                //cre->DeleteThreatList();
                if (!cre->GetThreatManager().IsThreatListEmpty())
                {
                    GuidSet Set;
                    for (auto ref : cre->GetThreatManager().GetUnsortedThreatList()) // .begin(); ref != cre->GetThreatManager().GetUnsortedThreatList().end(); ++ref) //  .GetModifiableThreatList().begin(); ref != cre->GetThreatManager().GetModifiableThreatList().end(); ++ref)
                    {
                        if (ref == NULL) continue;
                        if (ref->GetThreat() > 0.0f && ref->GetOwner() && ref->GetOwner()->GetGUID())
                            Set.insert(ref->GetOwner()->GetGUID());
                    ...
                    for (GuidSet::const_iterator i = Set.begin(); i != Set.end(); ++i)
                    {
                        ...
                        if (unit && (unit->IsFriendlyTo(me) || IsInBotParty(unit) || !unit->IsInCombat()))
                        {
                            cre->GetThreatManager().ClearThreat(unit);
                            // unit->GetThreatManager().ClearThreat(master);
                        }
                    }
                    /*
...
void bot_ai::OnSpellHit(Unit* caster, SpellInfo const* spell)
{
    for (uint8 i = 0; i != MAX_SPELL_EFFECTS; ++i)
    {
        ...
        if (auraname == SPELL_AURA_MOUNTED)
        {
            ...
            else if (master->HasAuraType(SPELL_AURA_MOD_INCREASE_MOUNTED_SPEED))
                me->SetSpeedRate(MOVE_RUN, master->GetSpeedRate(MOVE_RUN) * 1.37f);
            else
                me->SetSpeedRate(MOVE_RUN, master->GetSpeedRate(MOVE_RUN) * 1.25f);
    ...
    if (player->GetGUID().GetCounter() != creature->GetBotAI()->GetBotOwnerGuid())
    {
        if (creature->IsFreeBot() && !player->IsGameMaster())
        {
            ...
            if (!reason)
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_TAXI, "Will you follow me?", GOSSIP_SENDER_HIRE, GOSSIP_ACTION_INFO_DEF + reason);

            if (creature->GetBotClass() >= BOT_CLASS_EX_START)
                AddGossipItemFor(player, GOSSIP_ICON_TALK, "<Take a better look on this one>", GOSSIP_SENDER_SCAN, GOSSIP_ACTION_INFO_DEF + 1);
        ...
        if (player == creature->GetBotOwner() || (gr && gr->IsMember(creature->GetBotOwner()->GetGUID())))
        {
            switch (creature->GetBotClass())
            {
                case BOT_CLASS_MAGE:
                    ...
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "I need drink", GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
                    menus = true;
                    break;
                default:
                    break;
    ...
    switch (sender)
    {
        ...
        case GOSSIP_SENDER_HIRE:
        {
            ...
            else if (reason == -1)
            {
                ...
                BotSay("I just got back from a raid. Please give me time to rest.", player);
                break;
            }
            else
            {
...
void bot_minion_ai::SummonBotsPet(uint32 entry)
{
    ...
    //TempSummon* m_botsPet2 = me->SummonCreature(entry, x, y, z, 0, TEMPSUMMON_DEAD_DESPAWN);
...
void bot_minion_ai::OnOwnerDamagedBy(Unit* attacker)
{
    ...
    if (attacker && !attacker->IsWithinDist(me, maxdist))
        return;
            ...
            case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
                if (enchant_spell_id)
                {
                    ...
                    if (basepoints)
                        ...
                        me->CastSpell(enchant_spell_id, true, item);
                }
                break;
...
void bot_minion_ai::ApplyItemEquipSpell(Item* item, bool apply)
{
    ...
    for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (apply)
            me->CastSpell(me, spellInfo->Id, item);
        else
...
void bot_ai::FindMaster(bool force)
{
    ...
    if (Player* player = ObjectAccessor::FindPlayerByLowGUID(_ownerGuid)) // sObjectMgr->GetPlayerByLowGUID(_ownerGuid))
    {
...
void bot_minion_ai::SavePosition()
{
    ...
    {
        const_cast<CreatureData*>(data)->spawnPoint.m_mapId = mapid;
        const_cast<CreatureData*>(data)->spawnPoint.m_positionX = x; // posX = x;
        const_cast<CreatureData*>(data)->spawnPoint.m_positionY = y;
        const_cast<CreatureData*>(data)->spawnPoint.m_positionZ = z;
        const_cast<CreatureData*>(data)->spawnPoint.m_orientation = o;
    }
...
void bot_ai::BotSpeak(std::string const& text, uint8 msgtype, uint32 language, ObjectGuid speaker, ObjectGuid receiver)
{
    ...
    else
    {
        if (Unit* snd = ObjectAccessor::FindConnectedPlayer(speaker))
        {
            ...
            Cell::VisitWorldObjects(snd, notifier, dist);
...
void bot_minion_ai::CastBotItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx, Item* item, ItemTemplate const* proto, Spell const* /*spell*//* = NULL*/)
{
    ...
    if (procVictim & PROC_FLAG_TAKEN_DAMAGE)
    {
        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
        {
            ...
            if (roll_chance_f(chance))
                me->CastSpell(target, spellInfo->Id, item);
        }
    }
    ...
    for (uint8 e_slot = 0; e_slot != MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (entry)
            {
                if (entry->ProcsPerMinute) //   PPMChance)
            ...
            if (roll_chance_f(chance))
            {
                ...
                {
                    SpellCastTargets targets;
                    targets.SetOrigUnitTarget(me);
                    CustomSpellValues* Temp = nullptr;
                    me->CastSpell(targets, spellInfo->Id, item);
                }
                else
                {
                    me->CastSpell(target, spellInfo->Id, item);
