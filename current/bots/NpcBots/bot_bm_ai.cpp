...
public:
    ...
    public:
        ...
        void EnterEvadeMode(EvadeReason /* why */) { bot_minion_ai::EnterEvadeMode(false); }
        ...
        void MirrorImageFinish()
        {
            ...
            {
                Creature* illusion = master->SummonCreature(me->GetEntry(), *me, TEMPSUMMON_TIMED_DESPAWN, 1 * MINUTE * IN_MILLISECONDS)->ToCreature();
                if (!illusion)
                    continue;
            ...
            me->GetThreatManager().ClearAllThreat(); //.deleteReferences();
        ...
        void CriticalStrikeFinish(ObjectGuid targetGuid, CalcDamageInfo* dinfo, bool /*windwalk*/)
        {
            ...
            me->SendSpellNonMeleeDamageLog(target, CRITICAL_STRIKE_1,
                dinfo->Damages[0].Damage + dinfo->Damages[0].Absorb + dinfo->Damages[0].Resist + dinfo->Blocked,
                SPELL_SCHOOL_MASK_NORMAL, dinfo->Damages[0].Absorb, dinfo->Damages[0].Resist, true, dinfo->Blocked, true);
            // me->SendSpellNonMeleeDamageLog(dinfo);
            CleanDamage cl(0, 0, BASE_ATTACK, MELEE_HIT_CRIT);
            me->DealDamage(me, target, dinfo->Damages[0].Damage, &cl);
        ...
        void DamageDealt(Unit* victim, uint32& damage, DamageEffectType damageType)
        {
            ...
            if (IsTempBot())
            {
                ...
                if (victim->GetTypeId() != TYPEID_PLAYER)
                    victim->GetThreatManager().AddThreat(me, float(damage * 2));
