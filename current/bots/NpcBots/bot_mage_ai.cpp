...
public:
    ...
    struct mage_botAI : public bot_minion_ai
    {
        ...
        void EnterEvadeMode(EvadeReason /* why */) override { bot_minion_ai::EnterEvadeMode(false); }
                    ...
                    switch (spell->Effects[i].Effect)
                    {
                        ...
                        case SPELL_EFFECT_CREATE_ITEM_2:
                        {
                            ...
                            if (newitemid)
                            {
                                ...
                                if (count)
                                {
                                    Item* pItem = pTarget->StoreNewItem(dest, newitemid, true, GenerateItemRandomPropertyId(newitemid)); // Item::GenerateItemRandomPropertyId(newitemid));
                                    if (!pItem)
                                    {
...
/*exc.*/bool Polymorph, clearcast;
        ...
        enum MageSpecial
        {
            ...
            {
                return ::mage_bot::OnGossipHello(player, me);
            }
