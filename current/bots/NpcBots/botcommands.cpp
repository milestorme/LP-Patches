...
public:
    ...
    static bool HandleNpcSetOwnerCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (guidlow)
            sCharacterCache->GetCharacterNameByGuid(ObjectGuid(HighGuid::Player, guidlow), characterName);
        // sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HighGuid::Player, guidlow), characterName);
        else
            guidlow = sCharacterCache->GetCharacterGuidByName(characterName); // sObjectMgr->GetPlayerGUIDByName(characterName);
    ...
    static bool HandleNpcBotSpawnCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!creature->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, chr->GetPhaseMaskForSpawn(), id, chr->GetPosition()))
        {
            delete creature;
            return false;
        }
