...
public:
    ...
    struct priest_botAI : public bot_minion_ai
    {
        ...
        void EnterCombat(Unit* u) override { bot_minion_ai::EnterCombat(u); }
        void Aggro(Unit*) { }
        void AttackStart(Unit*) override { }
        void KilledUnit(Unit*) override { }
        void EnterEvadeMode(EvadeReason /* why */) override { bot_minion_ai::EnterEvadeMode(EVADE_REASON_OTHER); }
        void MoveInLineOfSight(Unit* u) override { bot_minion_ai::MoveInLineOfSight(u); }
        void JustDied(Unit* u) override { bot_minion_ai::JustDied(u); }
        ...
        bool HealTarget(Unit* target, uint8 hp, uint32 diff) override
        {
...
*/
        ...
        void ApplyClassDamageMultiplierHeal(Unit const* victim, float& heal, SpellInfo const* spellInfo, DamageEffectType damagetype, uint32 stack) const override
        {
            ...
            {
                if (spellId == HEAL)
                    flat_mod += spellpower * 0.4f * me->CalculateDefaultCoefficient(spellInfo, damagetype) * stack * 1.88f; // *me->CalculateDefaultCoefficient(spellInfo, HEAL) * stack; // CalculateLevelPenalty(spellInfo) * stack;
        ...
        void ApplyClassCritMultiplierHeal(Unit const* victim, float& crit_chance, SpellInfo const* spellInfo, SpellSchoolMask schoolMask, WeaponAttackType /*attackType*/) const override
        {
            uint32 spellId = spellInfo->Id;
            uint8 lvl = me->getLevel();
  ...
  /*Talent*/lvl >= 50 ? InitSpellMap(VAMPIRIC_TOUCH_1) : RemoveSpell(VAMPIRIC_TOUCH_1);
        ...
        void ApplyClassPassives() override
        {
...
/*Misc*/bool Devcheck;
  ...
  /*Talent*/VAMPIRIC_TOUCH_1                    = 34914
        ...
        enum PriestSpecial
        {
            ...
            {
                return ::priest_bot::OnGossipHello(player, me);
            }
